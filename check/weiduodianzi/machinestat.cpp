...
void MachineStat::updateSampleVal(quint32 val)
{
	...
	int timeConst = DataBase::getInstance()->queryData("timeconst").toInt();//时间常数;
	timeConst++;
	...
	if(m_machineStat.m_nCurrentWave == 0)//波长1
	{
		...
		else
		{
			...
			if(sampleValList.count() < timeConst*AU_LIST_MAX)
			{
				sampleValList << temp;
			...
			if(sampleValOutList.count() > timeConst)
	...
	else//波长2
	{
		...
		else
		{
			...
			if(sampleVal2List.count() < timeConst*AU_LIST_MAX)
			{
				sampleVal2List << temp;
			...
			if(sampleVal2OutList.count() > timeConst)
...
void MachineStat::updateRefVal(quint32 val)
{
	...
	if(m_machineStat.m_nCurrentWave == 0)//波长1;
	{
		...
		else
		{
			...
			if(refValList.count() < timeConst*AU_LIST_MAX)
			{
				refValList << temp;
			...
			if(refValOutList.count() > timeConst)
...
//========================================================================================
        ...
        emit(updateAuValue(m_machineStat.m_nSampleVal, m_machineStat.m_nRefVal, localAu1, m_machineStat.m_nCurrentWave));
	}
	else
	{
		if(m_machineStat.m_nUploadWhich == 1)//只上传s
		...
		else
		{
			...
			if(refVal2List.count() < timeConst*AU_LIST_MAX)
			{
				refVal2List << temp;
			...
			if(refVal2OutList.count() > timeConst)
...
//========================================================================================
        ...
        emit(updateAuValue(m_machineStat.m_nSampleVal2, m_machineStat.m_nRefVal2, localAu2, m_machineStat.m_nCurrentWave));
	}
}
